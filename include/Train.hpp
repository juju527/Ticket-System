...
class Train {
  public:
    ...
    char type;
    Train() { stationNum = 0; }
    Train(TrainID, int, Station[maxStationNum], int, int[maxStationNum], Time,
          int[maxStationNum], int[maxStationNum], Date, Date, char);
...
struct Info2 {
    ...
    bool operator==(const Info2 &other) const {
...
class TrainManager {
  ...
  public:
    TrainManager(BPT<TrainID, Train, 50> *, MemoryRiver<Tickets, 1> *,
                 BPT<TrainID, int, 100> *,
                 BPT<pair<Station, Station>, int, 100> *,
                 BPT<Station, int, 100> *, BPT<Station, Info2, 100> *);
    void clear();
    void add_train(TrainID, int, int, string, string, Time, string, string,
                   Date, Date, char);
