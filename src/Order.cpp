...
OrderManager::OrderManager(BPT<Username, bool, 100> *bpt2,
                           MemoryRiver<Tickets, 1> *mem1,
                           BPT<TrainID, int, 100> *bpt4,
                           MemoryRiver<Order, 1> *mem2,
                           BPT<Username, int, 100> *bpt8,
                           BPT<pair<TrainID, Date>, int, 50> *bpt9) {
    this->bpt2 = bpt2;
    this->mem1 = mem1;
    this->bpt4 = bpt4;
    this->mem2 = mem2;
...
void OrderManager::buy_ticket(Username username, TrainID trainID, Date date,
                              int num, Station s, Station t, bool op) {
    vector<int> aux = bpt4->find(trainID);
    if (!bpt2->find(username).size() || !aux.size()) {
        cout << -1 << endl;
        return;
    }
    Tickets tickets;
    ...
    for (int i = 0; i < train.stationNum; i++) {
        if (train.stations[i] == s) {
            ...
            r = i;
            od.r = r;
            od.arrivingDate = td + p;
            od.arrivingTime = tt;
        }
    }
    if (l == -1 || r == -1 || l > r || num > train.seatNum) {
        cout << -1 << endl;
        return;
    }
    int seat = 1e9;
    for (int i = l; i < r; i++)
        seat = std::min(seat, tickets.seat[p][i]);
    if (seat >= num) {
        int cost = train.prices[r] - train.prices[l];
        for (int i = l; i < r; i++)
            tickets.seat[p][i] -= num;
...
void OrderManager::query_order(Username username) {
    ...
    for (int i = aux.size() - 1; i >= 0; i--) {
        ...
        cout << '[' << STATUS[od.status] << "] " << od.trainID << " " << od.s
             << " " << date_time_to_str(od.leavingDate, od.leavingTime)
             << " -> " << od.t << " "
             << date_time_to_str(od.arrivingDate, od.arrivingTime) << " "
             << od.price << " " << od.num << endl;
    }
    return;
}
...
void OrderManager::refund_ticket(Username username, int num) {
    ...
    Train train = tickets.train;
    for (int i = od.l; i < od.r; i++)
        tickets.seat[od.d0][i] += od.num;
    vector<int> tmp = bpt9->find(pair(od.trainID, od.d0));
    for (int i = 0; i < tmp.size(); i++) {
        Order od1;
        mem2->readorder(od1, tmp[i]);
        int seat = 1e9;
        for (int j = od1.l; j < od1.r; j++)
            seat = std::min(seat, tickets.seat[od1.d0][j]);
        if (seat >= od1.num) {
            for (int j = od1.l; j < od1.r; j++)
                tickets.seat[od1.d0][j] -= od1.num;
