...
Train::Train(TrainID trainID, int stationNum, Station stations[maxStationNum],
             int seatNum, int prices[maxStationNum], Time startTime,
             int travelTimes[maxStationNum], int stopoverTimes[maxStationNum],
             Date sales, Date salet, char type) {
...
bool Tickets::operator<(const Tickets &other) { return train < other.train; }
bool Tickets::operator==(const Tickets &other) { return train == other.train; }

// TrainManager 实现

TrainManager::TrainManager(BPT<TrainID, Train, 50> *bpt3,
                           MemoryRiver<Tickets, 1> *mem1,
                           BPT<TrainID, int, 100> *bpt4,
                           BPT<pair<Station, Station>, int, 100> *bpt5,
                           BPT<Station, int, 100> *bpt6,
...
void TrainManager::add_train(TrainID trainID, int stationNum, int seatNum,
                             string stations, string prices, Time startTime,
                             string travelTimes, string stopoverTimes,
                             Date sales, Date salet, char type) {
    if (bpt3->find(trainID).size()) {
        cout << -1 << endl;
        return;
    }
    ...
    for (int i = 0; i < prices.size(); i++) {
        if (prices[i] == '|')
            _prices[j++] = (sum += str_to_int(aux)), aux = "";
        else
            aux += prices[i];
    }
    if (aux.size())
        _prices[j++] = (sum += str_to_int(aux));

    aux = "", j = 1, sum = 0;
    for (int i = 0; i < travelTimes.size(); i++) {
        if (travelTimes[i] == '|')
            _travelTimes[j++] = (sum += str_to_int(aux)), aux = "";
        else
            aux += travelTimes[i];
    }
    if (aux.size())
        _travelTimes[j++] = (sum += str_to_int(aux));
    aux = "", j = 1, sum = 0;
    for (int i = 0; i < stopoverTimes.size(); i++) {
        if (stopoverTimes[i] == '|')
            _stopoverTimes[j++] = (sum += str_to_int(aux)), aux = "";
        else
            aux += stopoverTimes[i];
    }
    if (aux.size())
        _stopoverTimes[j++] = (sum += str_to_int(aux));

    Train train(trainID, stationNum, _stations, seatNum, _prices, startTime,
                _travelTimes, _stopoverTimes, sales, salet, type);
...
void TrainManager::delete_train(TrainID trainID) {
    ...
    if (!tmp.size() || bpt4->find(trainID).size()) {
        cout << -1 << endl;
        return;
    }
...
void TrainManager::query_train(TrainID trainID, Date date) {
    ...
    Train train = tmp[0];
    if (train.sales > date || train.salet < date) {
        cout << -1 << endl;
        return;
    }
    ...
    if (aux.size()) {
        ...
        cout << train.stations[0] << " xx-xx xx:xx -> "
             << date_time_to_str(date, train.startTime) << " 0 "
             << tickets.seat[D][0] << endl;
        for (int i = 1; i < train.stationNum - 1; i++) {
            cout << train.stations[i] << " "
                 << date_time_calc(date, train.startTime,
                                   train.travelTimes[i] +
                 ...
                 << " " << train.prices[i] << " " << tickets.seat[D][i] << endl;
        }
        cout << train.stations[train.stationNum - 1] << " "
             << date_time_calc(date, train.startTime,
                               train.travelTimes[train.stationNum - 1] +
                                   train.stopoverTimes[train.stationNum - 2])
             << " -> xx-xx xx:xx " << train.prices[train.stationNum - 1] << " x"
             << endl;
        return;
    }
    cout << train.stations[0] << " xx-xx xx:xx -> "
         << date_time_to_str(date, train.startTime) << " 0 " << train.seatNum
         << endl;
    for (int i = 1; i < train.stationNum - 1; i++) {
        cout << train.stations[i] << " "
             << date_time_calc(date, train.startTime,
             ...
             << " -> "
             << date_time_calc(date, train.startTime,
                               train.travelTimes[i] + train.stopoverTimes[i])
             << " " << train.prices[i] << " " << train.seatNum << endl;
    }
    cout << train.stations[train.stationNum - 1] << " "
         << date_time_calc(date, train.startTime,
                           train.travelTimes[train.stationNum - 1] +
...
struct Info1 {
    ...
    Time Lt, At;
    int price, time, seat;
    Info1(TrainID _trainID, Date _Ld, Time _Lt, Date _Ad, Time _At, int _price,
          int _time, int _seat) {
        trainID = _trainID;
...
void TrainManager::query_tickets(Station s, Station t, Date date, bool op) {
    ...
    for (int i = 0; i < tmp.size(); i++) {
        ...
        if (sd > date || sd + train.salet - train.sales < date)
            continue;
        int p = date - sd;
        td += p;
        sd = date;
        ...
        Info1 aux(train.trainID, sd, st, td, tt,
                  train.prices[r] - train.prices[l], rt - lt, seat);
        res.push_back(aux);
    }
    if (!op) {
        ...
        while (q.size()) {
            ...
            q.pop();
            cout << cur.trainID << " " << s << " "
                 << date_time_to_str(cur.Ld, cur.Lt) << " -> " << t << " "
                 << date_time_to_str(cur.Ad, cur.At) << " " << cur.price << " "
                 << cur.seat << endl;
        }
    } else {
...
Info2::Info2(Date _Ld, Time _Lt, Date _Ad, Time _At, int _t, int _c, int _seat,
             TrainID _trainID) {
    this->Ld = _Ld;
    this->Lt = _Lt;
    this->Ad = _Ad;
...
struct Info3 {
    ...
    int price1, price2;
    int seat1, seat2;
    Info3(int _t, int _c) { time = _t, cost = _c; }
    Info3(int _t, int _c, TrainID t1, TrainID t2, Station _transfer, Date _Ld1,
          Time _Lt1, Date _Ad1, Time _At1, Date _Ld2, Time _Lt2, Date _Ad2,
          Time _At2, int p1, int p2, int s1, int s2) {
        time = _t, cost = _c;
...
bool Cmp0(const Info3 &i, const Info3 &j) {
    ...
    if (!(i.trainID1 == j.trainID1))
        return i.trainID1 < j.trainID1;
...
void TrainManager::query_transfer(Station s, Station t, Date date, bool op) {
    ...
    for (int i = 0; i < A.size(); i++) {
        ...
        if (sd > date || sd + train1.salet - train1.sales < date)
            continue;
        ...
        for (int j = l + 1; j < train1.stationNum; j++) {
            ...
            Info2 info(date, st, d + p, t, delta - d0, cost, seat,
                       train1.trainID);
    ...
    for (int i = 0; i < B.size(); i++) {
        ...
        for (int j = 0; j < train2.stationNum; j++)
            if (train2.stations[j] == t) {
                if (j)
                    arrivet =
                        train2.travelTimes[j] + train2.stopoverTimes[j - 1];
                r = j;
                break;
            }
        for (int j = r - 1; j >= 0; j--) {
            ...
            for (auto info : aux) {
                if (info.trainID == train2.trainID)
                    continue;
                if (td + train2.salet - train2.sales < info.Ad)
                    continue;
                if (td + train2.salet - train2.sales == info.Ad && tt < info.At)
                    continue;
                Date Td = td;
                Time Tt = tt;
                if (td <= info.Ad) {
                ...
                int t0 = info.t + calc_interval(info.Ad, info.At, Td, Tt) +
                         arrivet - delta;
                int c0 = info.c + train2.prices[r] - train2.prices[j];
                int seat = 1e9;
                for (int k = j; k < r; k++)
                    seat = std::min(seat, tickets.seat[Td - td][k]);
                Info3 cur(t0, c0, info.trainID, train2.trainID,
                          train2.stations[j], info.Ld, info.Lt, info.Ad,
                          info.At, Td, Tt, date_calc(Td, Tt, arrivet - delta),
                          time_calc(Td, Tt, arrivet - delta), info.c,
                          train2.prices[r] - train2.prices[j], info.seat, seat);
                if (!op) {
                    if (Cmp0(cur, res))
                        res = cur;
                } else {
    ...
    cout << res.trainID1 << " " << s << " "
         << date_time_to_str(res.Ld1, res.Lt1) << " -> " << res.transfer << " "
         << date_time_to_str(res.Ad1, res.At1) << " " << res.price1 << " "
         << res.seat1 << endl;
    cout << res.trainID2 << " " << res.transfer << " "
         << date_time_to_str(res.Ld2, res.Lt2) << " -> " << t << " "
         << date_time_to_str(res.Ad2, res.At2) << " " << res.price2 << " "
         << res.seat2 << endl;
    return;
}
