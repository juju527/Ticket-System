...
void UserManager::add_user(Username cur, Username username, Password password, Name name, MailAddress mailaddress, int privilege){
    ...
    if(aux.privilege>privilege)bpt1->ins(username,user),cout<<0<<endl;
    else cout<<-1<<endl;
    return ;
}
...
void UserManager::modify_profile(Username cur, Username username,string st){
    ...
    for(int i=2;i<str.size();i+=2){
        if(str[i]=="-p")u.password=str[i+1];
        else if(str[i]=="-n")u.name=str[i+1];
        else if(str[i]=="-m")u.mailaddress=str[i+1];
        else if(str[i]=="-g")u.privilege=str_to_int(str[i+1]),tag=1;
    }
    if(tag&&c.privilege<=u.privilege){cout<<-1<<endl;return ;}
    bpt1->del(username,buf);bpt1->ins(username,u);
    cout<<u.username<<" "<<u.name<<" "<<u.mailaddress<<" "<<u.privilege<<endl;
    return ;
}