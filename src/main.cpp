        ...
        if (v[1] == "add_user") {
            ...
            for (int i = 2; i < v.size(); i += 2) {
                if (v[i] == "-i")
                    ...
                    stationNum = str_to_int(v[i + 1]);
                else if (v[i] == "-m")
                    seatNum = str_to_int(v[i + 1]);
                else if (v[i] == "-s")
                    stations = v[i + 1];
                else if (v[i] == "-p")
                    prices = v[i + 1];
                else if (v[i] == "-x")
                    startTime = str_to_time(v[i + 1]);
                else if (v[i] == "-t")
                    travelTimes = v[i + 1];
                else if (v[i] == "-o")
                    stopoverTimes = v[i + 1];
                    ...
                    for (int j = 0; j < v[i + 1].size(); j++)
                        if (v[i + 1][j] == '|') {
                            ...
                            s += v[i + 1][j];
                    for (int j = p; j < v[i + 1].size(); j++)
                        t += v[i + 1][j];
                    sales = str_to_date(s), salet = str_to_date(t);
                } else if (v[i] == "-y")
                    type = v[i + 1][0];
            }
            TM.add_train(trainID, stationNum, seatNum, stations, prices,
                         startTime, travelTimes, stopoverTimes, sales, salet,
                         type);
